...
scheduler(void)
{
  ...
  for(;;){
    ...
    uint64 golden_ticket = (randomn_gen()%tottickets)+1;
    struct proc* lmao;
    int changeflag=0;
    uint64 count =0;
    ...
    uint64 minpriority=0;
    struct proc *minproc = 0;
    int changeflag=0;
    // p=proc;
    // for(int i = 0;p<&proc[NPROC];p++, i++)
    // {
    //   printf("%d lma\n", i);
    // }
    p=proc;
    for(;p<&proc[NPROC];p++)
    {
      ...
      if ( changeflag == 0 ) {
        ...
        if (DP == minpriority) {
          if (minproc->calls == p->calls) {
            if (p->start_time < minproc->start_time) {
              release(&minproc->lock);
              minproc = p;
            }
            else {
...
#endif
#ifdef ROUNDROBIN
    printf("RR %d\t%s    %d\t  %d\t%d", p->pid, state, p->rtime, time, p->calls);
#endif
#ifdef LBS
  state=NULL;
  time =0;
